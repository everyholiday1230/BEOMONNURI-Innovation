...
(function() {
  ...
  // 상태
  const P = { sec: 'mine', code: '', points: 0, referrals: 0, paid: 0, history: [], shopCat: 'ai', loaded: false, error: false };
  let _busy = false;
  ...
  async function load() {
    ...
    setStatus('loading');
    try {
      const [c, pts, hist] = await Promise.all([
        fetchJson(`${API}/v1/referral/my-code`),
        fetchJson(`${API}/v1/referral/points`),
        fetchJson(`${API}/v1/referral/history`),
      ]);
      if (c && c.success && c.data) P.code = c.data.code || '';
      if (pts && pts.success && pts.data) { P.points = pts.data.points || 0; P.referrals = pts.data.referrals || 0; P.paid = pts.data.paid_referrals || 0; }
      else { P.error = true; }
    ...
    return `
      <div class="pt-balance">
        ...
        <div class="pt-balance-grid">
          <div class="row"><span class="k">사용 가능 포인트</span><span class="v">${fmtP(P.points)} P</span></div>
          <div class="row"><span class="k">소멸 예정 포인트</span><span class="v">연동 예정</span></div>
          <div class="row"><span class="k">이번 달 적립</span><span class="v">${fmtP(P.history.filter(h => h.amount > 0 && isThisMonth(h.date)).reduce((a, h) => a + h.amount, 0))} P</span></div>
          <div class="row"><span class="k">이번 달 사용</span><span class="v">${fmtP(Math.abs(P.history.filter(h => h.amount < 0 && isThisMonth(h.date)).reduce((a, h) => a + h.amount, 0)))} P</span></div>
        </div>
        <div class="pt-phase2">소멸 예정 포인트는 포인트 유효기간(소멸 버킷) 연동 후 표시됩니다.</div>
      </div>
  ...
  // 3) 포인트 상점
  function secShop() {
    const list = SHOP.filter(s => s.cat === P.shopCat);
    return `
      ...
      <p class="pt-phase2">상품 구매·이용권 지급은 포인트 상점 백엔드 연동(Phase 2) 후 활성화됩니다. 현재는 상품 구성과 필요 포인트를 안내합니다.</p>`;
  }
  function prodCard(p) {
    const short = isLoggedIn() && P.points < p.cost;
    return `<div class="pt-prod">
      <div class="pt-prod-info"><div class="pt-prod-name">${esc(p.name)}</div><div class="pt-prod-desc">${esc(p.desc)}</div><div class="pt-prod-meta">이용 기간 ${esc(p.period)}</div></div>
      <div class="pt-prod-right"><div class="pt-prod-price">${fmtP(p.cost)} P</div>${short ? '<div class="pt-prod-short">포인트 부족</div>' : ''}
        <button class="pt-btn pt-btn-primary pt-btn-xs" type="button" style="margin-top:6px" onclick="window.PointsUI.buy('${esc(p.name)}', ${p.cost})">구매</button></div>
    </div>`;
  }
  ...
  function secAiPass() {
    return `<div class="pt-card"><div class="pt-card-title">AI 분석 이용권</div>
      ...
      <div class="pt-prod" style="margin-top:10px">
        ...
        <div class="pt-prod-right"><div class="pt-prod-price">1,000 P</div><button class="pt-btn pt-btn-primary pt-btn-xs" type="button" style="margin-top:6px" onclick="window.PointsUI.buy('AI 심화 분석 1회', 1000)">이용하기</button></div>
      </div>
  ...
  function confirmBuy(featureName, cost) {
    ...
    // Phase 2: 실제 포인트 차감 + 기능 지급 엔드포인트 연동 예정. 현재는 안내만.
    setTimeout(() => {
      _busy = false;
      closeModal();
      window.showToast && window.showToast('포인트 사용(기능 지급)은 곧 제공됩니다. 보유 포인트는 차감되지 않았습니다.', '#921230');
    }, 400);
  }
  ...
  window.PointsUI = { reload: load, go, copy, share, buy, confirmBuy, closeModal, openPane, getState: () => P };
