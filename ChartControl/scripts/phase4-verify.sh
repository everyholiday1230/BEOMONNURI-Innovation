#!/usr/bin/env bash
# Phase 4 verification runner. Writes artifacts/logs/phase4-<name>.log with a header
...
DIRTY="$(test -n "$(git status --porcelain)" && echo 'dirty(phase-4 delta)' || echo 'clean')"
ENVLINE="Node $(node -v) · pnpm $(pnpm -v) · $(uname -srm)"

run() {
  local name="$1"; shift; local cmd="$*"; local log="$LOGDIR/phase4-${name}.log"; local start end code
...
echo "### Phase 4 verification — git $GITSHA ($DIRTY)"
