#!/usr/bin/env bash
# Phase 5 Admin UI Closure verification runner. Writes artifacts/logs/phase5-closure-<name>.log
# with a header (command/env/git SHA/start/end/exit) + raw output. Includes test:admin + e2e:admin.
set -u
cd "$(dirname "$0")/.." || exit 2
LOGDIR="artifacts/logs"; mkdir -p "$LOGDIR"
GITSHA="$(git rev-parse HEAD 2>/dev/null || echo unknown)"
DIRTY="$(test -n "$(git status --porcelain)" && echo 'dirty(phase-5-closure delta)' || echo 'clean')"
ENVLINE="Node $(node -v) · pnpm $(pnpm -v) · $(uname -srm)"

run() {
  local name="$1"; shift; local cmd="$*"; local log="$LOGDIR/phase5-closure-${name}.log"; local start end code
...
echo "### Phase 5 Admin UI Closure verification — git $GITSHA ($DIRTY)"
...
run admin-test  "pnpm test:admin"
run admin-e2e   "pnpm e2e:admin"
echo "### done"
