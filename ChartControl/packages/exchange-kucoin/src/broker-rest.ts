...
export class KucoinBrokerClient {
   ...
   *
   * ★★ 돌려주는 숫자는 **평생 누적**이다. 과거에 서명이 빠졌던 기간이 있으면 그
   *   무태그 실적이 계속 섞여 보인다. 실측(2026-09-19): 어떤 이용자의 무태그
   *   선물 거래량이 14,663 USDT 였는데 전부 2026-08-23~27 것이었고, 08-30 이후는
   *   매일 무태그 0 이었다. **누적값만 보고 "지금 서명이 안 붙는다" 고 판단하면
   *   틀린다** — 기간별 추이는 `getCommission()` 으로 봐야 한다.
   *
   * ★★★ `startAt`/`endAt` 은 **거래 시각으로 걸리지 않는다.** 실측: 최근 3일·10일
   *   범위로 부르면 **0명** 이 왔는데, 같은 기간에 실제로 거래가 있었다
   *   (`getCommission` 은 09-18 에 10,547 USDT 태그 거래량을 보여줬다).
   *   즉 이 필터를 넣으면 **활동 중인 거래자가 사라진다.** 넣지 말 것.
   */
  async getUserList(
    operator: UserCredentials,
    broker: Partial<BrokerCredentials> | null,
