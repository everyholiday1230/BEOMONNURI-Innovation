  ...
  .object({
    ...
    /*
       ★★ 필수 동의(약관·개인정보·위험고지).
         ...
         이 필드가 스키마에 없어서 zod 가 **조용히 버렸다.** 화면은 체크박스를 강제하고
         값도 보내는데, 서버는 받지 못해 기록할 것이 없었다. 결과: 사용자 38명 중 동의
         기록 0건. 동의를 받았다는 입증 책임은 우리에게 있으므로 기록이 없으면 받지 않은
         것과 같다.

       ★ 필수로 두지 않는다(optional). 이유: 기존 클라이언트나 다른 경로(구글 가입)가
         이 값을 보내지 않을 수 있고, 그때 가입 자체가 막히면 손해가 더 크다. 대신
         **값이 참일 때만 기록**하고, 거짓·부재는 기록하지 않는다 — 없는 동의를 만들지
         않는다.
    */
