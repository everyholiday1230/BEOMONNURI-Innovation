...
import { MailSendError, ResendMailProvider, renderMail, resendFromEnv, type ResendConfig } from '../resend-mail';
...
describe('MAIL-01 the body contains an actual link', () => {
  it('[1] a verification message renders a link to /verify-email?token=', () => {
    const r = renderMail(VERIFY, 'https://app.example.com');
    ...
    expect(r.subject).toBe('이메일 인증');
  });

  it('[2] a reset message renders a link to /password-reset?token=', () => {
    const r = renderMail(RESET, 'https://app.example.com');
    ...
    // ignoring it is safe.
    expect(r.text).toMatch(/본인이 요청하지 않았다면/u);
  });

  it('[3] a trailing slash on the base URL does not double up', () => {
    expect(renderMail(VERIFY, 'https://app.example.com/').text).toContain('https://app.example.com/verify-email?');
  });

  it('[4] the token is URL-encoded', () => {
    const r = renderMail({ ...VERIFY, meta: { token: 'a b/c+d', kind: 'verify' } }, 'https://x.com');
  ...
  it('[5] an unrecognised kind falls back to the caller text rather than a wrong template', () => {
    const r = renderMail({ to: 'a@b.c', subject: 'Notice', text: 'Hello', meta: { kind: 'something-new' } }, 'https://x.com');
  ...
  it('[6] a missing token falls back rather than producing a link to nothing', () => {
    const r = renderMail({ ...VERIFY, meta: { kind: 'verify' } }, 'https://x.com');
  ...
  it('[7] HTML is escaped so a hostile subject cannot inject markup', () => {
    const r = renderMail({ to: 'a@b.c', subject: '<img src=x onerror=alert(1)>', text: 'x' }, 'https://x.com');
