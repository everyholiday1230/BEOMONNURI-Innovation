...
const STEP_MIN: Record<Timeframe, number> = {
  ...
  '4h': 240,
  /*
     ★★ 아래 네 개는 **BitMart 로 검증하지 않았다.** BitMart 는 프로덕션에서 비활성이다
       (`BITMART_PRODUCTION_DISABLED`). 분 단위 환산은 산술적으로 맞지만 거래소가 그
       step 을 받아주는지는 확인하지 않았다 — 받지 않으면 요청이 실패한다.
     ★ 0 이나 임의의 값으로 채우지 않는다. 틀린 step 은 **다른 주기의 봉**을 가져와
       조용히 잘못된 차트를 만든다. 실패하는 것이 낫다.
  */
  '6h': 360,
  '8h': 480,
  '12h': 720,
