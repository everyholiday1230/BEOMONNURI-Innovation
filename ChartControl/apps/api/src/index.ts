  ...
  try {
    ...
    */
    ...
    try {
      ...
      */
      /*
         ★★ 일일 손실 한도는 **걸지 않는 것이 기본**이다 (운영 결정, 2026-09-08).

           우리는 분석 소프트웨어를 공급하고 주문은 고객이 자기 계정·자기 판단으로
           낸다. 손실 한도는 고객의 자금 관리 영역이고, 강제 종료는 거래소가 청산으로
           처리한다.

         ★ 그래도 값을 넣으면 즉시 켜진다. 그때는 **반쪽만 덮인다는 것**을 알아야 한다 —
           실현손익은 KuCoin history-positions(선물)에서만 읽을 수 있고, 현물은 그
           경로가 없다. 그리고 측정에 실패하면 주문이 거부된다(막는 쪽이 정직하다).

           반쪽 보호를 걸어 두고 화면에 "손실 한도 있음" 이라고 말하는 것이 더 나쁘다.
           그래서 켤 때마다 이 경고를 남긴다.
      */
      if (env.tradingPolicy.dailyLossLimit && Number(env.tradingPolicy.dailyLossLimit) > 0) {
        console.warn(
          ...
          + '기본값은 "걸지 않음" 이다 — 누군가 TRADE_DAILY_LOSS_LIMIT 을 넣었다. '
          + '판정 근거는 거래소 실현손익(KuCoin history-positions)이며 **선물만** 덮는다. '
          + '현물 손실은 반영되지 않고, 측정에 실패하면 주문이 거부된다.',
        );
      } else {
        console.log(
          '[api] 일일 손실 한도: 걸지 않는다 — 고객이 자기 위험을 진다(거래소 청산이 최종 방어).',
        );
      }
          ...
          */
          ...
          /*
             ★★ 마케팅 수신 동의를 기록한다.
               ...
               화면은 체크박스를 보여주고 값도 보냈는데 받는 곳이 없어 **조용히
               버려졌다.** 동의를 받은 척만 하고 기록이 없었던 것이다 — 기록 없는
               동의는 받지 않은 것과 같고, 입증 책임은 우리에게 있다.

             ★ 필수 동의(agreed)와 **분리해서** 기록한다. 개인정보보호법 §22③④ 가
               별도로 받으라고 하는 것이 이것이다. 하나로 묶으면 필수 동의만 한
               고객에게도 광고를 보내게 된다.

             ★ true 일 때만 쓴다. false·undefined 는 컬럼 기본값(FALSE)이 이미
               맞으므로 쓸 필요가 없고, 쓰지 않으면 마이그레이션 미적용 환경에서
               불필요한 실패도 없다.

             ★★ 실패해도 가입을 깨뜨리지 않는다. 이 컬럼은 마이그레이션 0045 로
               추가되는데 마이그레이션은 부팅 때 자동 실행되지 않는다(README).
               코드가 먼저 배포되면 컬럼이 없을 수 있고, 그때 가입이 500 으로
               죽으면 **광고 동의 하나 때문에 신규 고객을 전부 잃는다.**
               로그만 남기고 넘어간다 — 그러면 운영자가 마이그레이션 누락을 안다.
          */
          if (core.pool && regCtx && regCtx.marketingOptIn === true) {
            try {
              await core.pool.query(
                'UPDATE users SET marketing_opt_in = TRUE, marketing_opt_in_at = now() WHERE id = $1',
                [userId],
              );
              console.log(`[legal] 마케팅 수신 동의 기록 — user=${userId}`);
            } catch (e) {
              console.error(
                '[legal] ★ 마케팅 동의 기록 실패 — 가입은 유지한다. '
                + '마이그레이션 0045_marketing_consent 가 적용됐는지 확인할 것:',
                (e as Error).message,
              );
            }
          }
