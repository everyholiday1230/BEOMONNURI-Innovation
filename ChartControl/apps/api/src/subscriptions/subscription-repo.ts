...
export class PgSubscriptionRepo {
   ...
   *
   * ★★★ **아직 호출하는 곳이 없다.** PayPal 웹훅 엔드포인트를 만들지 않았다.
   *
   *   3525ea6 커밋에서 "멱등성 보장" 이라고 썼는데, 정확히는 **DB 와 함수만 준비된
   *   상태**다. 감사에서 이 점이 지적됐고 사실이다 — 주장이 앞서 있었다.
   *
   * ★ 왜 지금 웹훅을 붙이지 않는가 — **15분 폴링(subscription-reconcile)이 같은 일을
   *   한다.** 그리고 폴링에는 웹훅에 없는 장점이 있다:
   *     · PayPal 쪽 설정이 **전혀 필요 없다**(URL 등록·서명 검증 ID)
   *     · 유실되지 않는다(웹훅은 재시도가 소진되면 사라진다)
   *   실제로 오늘 샌드박스에서 "승인 후 브라우저가 돌아오지 않은 결제" 2건을 폴링이
   *   구제하는 것을 확인했다.
   *
   * ★★ 웹훅이 확실히 나은 경우는 **분쟁(chargeback)** 이다. 고객이 PayPal 에 이의를
   *   제기하면 폴링으로는 알 수 없다(구독 상태가 안 바뀐다). 그것이 필요해지면
   *   이 함수들을 그때 쓴다.
   *
   * ★ 그래서 **지우지 않고 남긴다.** 지우면 나중에 다시 만들어야 하고, 남겨 두면
   *   "왜 안 쓰나" 를 이 주석이 답한다. 다만 "이미 동작한다" 고 오해하지 않도록
   *   여기에 분명히 적는다.
   *
