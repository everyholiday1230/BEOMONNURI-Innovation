...
describe('CHART-LEGEND — 켜둔 지표의 이름이 보인다', () => {
  ...
  it('[2] 설정값을 함께 보여준다', () => {
    ...
    */
    /*
       ★ 2026-09-18: 표기 이름이 `name`(내부 키) → `shown`(사람이 읽을 이름)으로
         바뀌었다. 커스텀 지표·신호 규칙의 내부 키(`SIG_MACD_______U9NTX`)가
         레전드에 그대로 나오던 것을 고친 결과다.
       ★ 검사하는 것은 **설정값을 괄호로 함께 적는가** 이므로 변수명에 묶지 않는다.
    */
    expect(kline, '설정값을 괄호로 함께 적지 않는다')
      .toMatch(/\$\{(?:name|shown)\}\(\$\{params\.join\(','\)\}\)/);
    /* ★ 내부 키가 아니라 표시 이름을 쓰는지 함께 본다. */
    expect(kline, '표시 이름(title)을 우선하지 않는다')
      .toMatch(/d\.title \? String\(d\.title\) : name/);
  });
