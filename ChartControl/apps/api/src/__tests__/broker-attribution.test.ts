...
describe('거래소 자격 문서가 세 종류를 구분한다', () => {
  /*
     ★★★ 2026-09-19 운영자 질문: "이거 2개가 다야?"
       거래소가 메일로 준 것(브로커 태그·OAuth)만으로는 수익 조회가 안 된다.
       운영자 API 키는 **우리가 직접 만들어야** 하고 거래소는 주지 않는다.
       이걸 모르면 "준 걸 다 넣었는데 수익이 안 보인다" 에서 막힌다 — 실제로 막혔다.
  */
  const doc = read('docs/EXCHANGE-BROKER-CREDENTIALS.md');

  it('거래소가 주는 것과 우리가 만드는 것을 나눈다', () => {
    expect(doc, '운영자 키를 거래소가 준다고 오해할 수 있다')
      .toMatch(/\*\*거래소는 ②를 주지 않는다\.\*\*/u);
    expect(doc, '우리가 만든다는 사실이 없다').toMatch(/\*\*우리가 직접 만든다\*\*/u);
  });

  /*
     ★★ 권한을 과하게 주면 사고 시 피해가 커진다. 이 키는 보고서만 읽는다.
       공식 문서에 api-permission: General 로 적혀 있다.
  */
  it('읽기 전용 권한으로 충분하다고 적는다', () => {
    expect(doc, '필요 권한을 적지 않았다').toMatch(/General\(읽기 전용\)/u);
    expect(doc, '출금 권한을 주지 말라고 하지 않는다').toMatch(/거래·출금 권한은 주지 않는다/u);
  });

  /*
     ★★★ IP 화이트리스트를 걸면 Render 에서 즉시 막힌다(고정 IP 미구매).
       이 경고가 없으면 키를 다시 만들어야 한다.
  */
  it('IP 화이트리스트 금지를 경고한다', () => {
    expect(doc, 'IP 제한 경고가 없다').toMatch(/IP 화이트리스트를 걸지 않는다/u);
    expect(doc, '이유(고정 IP 아님)를 적지 않았다').toMatch(/고정 IP 가 아니다/u);
  });

  /*
     ★★ 400003 을 "키가 없다" 로 읽으면 엉뚱한 곳을 고친다. 서명이 틀려도 같은 코드다.
  */
  it('오류코드를 오해하지 않게 적는다', () => {
    /*
       ★ `400003` 부터 시작하는 문장 안쪽만 본다. 문서 전체에서 찾으면 다른 곳의
         같은 표현(PARTNER-VERIFY 설명)이 대신 통과시킨다 — 실제로 그랬다.
    */
    const m = /`400003[^`]*`[\s\S]{0,120}/u.exec(doc);
    expect(m, '400003 을 설명하는 문장이 없다').not.toBeNull();
    expect(m![0], '서명이 틀려도 같은 코드가 나온다는 사실이 없다').toMatch(/서명이 틀려도[\s\S]{0,12}나온다/u);
  });

  it('우리 플래그가 거래소 판정이 아니라고 적는다', () => {
    /* ★ 이 문서의 핵심. brokerAttached 를 확인으로 착각한 것이 이번 조사의 출발점이었다. */
    expect(doc, '심판이 거래소라는 사실이 없다').toMatch(/심판은 거래소다/u);
  });

  it('비밀값을 적지 않는다', () => {
    /* ★★★ 문서에 실제 키를 적으면 저장소가 비밀 보관소가 된다. 이름과 출처만 적는다. */
    expect(doc, '문서가 비밀값을 적지 않겠다고 선언하지 않았다')
      .toMatch(/\*\*비밀값은 적지 않는다\.\*\*/u);
    /* UUID 형태(브로커 key)와 32자 hex(OAuth secret)가 본문에 있으면 실패시킨다. */
    expect(doc.match(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/u), 'UUID 형태 비밀값이 있다').toBeNull();
