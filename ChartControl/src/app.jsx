      ...
      */
      setTimeout(() => {
        try {
          ...
          */
          /*
             ★ 저장 화면에서 넘어온 **그림**(`qt.pendingSavedItem`)은 여기서 처리하지
               않는다. 코파일럿이 자기 마운트에서 직접 읽는다.

               실측으로 배운 것: 차트 준비 시점에 코파일럿이 아직 마운트되지 않아
               `__qtApplySavedItem` 이 없었다. 그런데 여기서 **대기 항목을 이미 지워
               버려서** 그림이 사라졌다(대기값은 없어졌고 화면엔 아무것도 없다).
