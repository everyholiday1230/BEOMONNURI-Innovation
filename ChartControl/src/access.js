...
(function () {
  ...
  var PUBLIC_ROUTES = new Set([
    ...
    /*
       ...
       ★★★ `/refund` 와 `/company` 가 **빠져 있었다** — 프로덕션 실측(2026-09-17):

           #/refund   → 환불정책(2688자) + **div.auth-shell 로그인 폼(1007자)** 이
                        아래에 나란히 렌더됐다. 고객이 환불 정책을 읽는 화면에
                        로그인 폼이 붙어 있었다.
           #/company  → 사업자 정보 + **404 화면(956자)** 이 함께 렌더됐다.
                        전자상거래법 §10 표시 의무를 위해 푸터에서 링크하는 화면인데,
                        그 화면이 "찾을 수 없다" 를 함께 보여주고 있었다.

         원인은 이 목록 하나다. 공개가 아니면 `needsLogin`(또는 `isNotFound`)이 참이 되어
         **문서 컴포넌트와 로그인/404 컴포넌트가 동시에** 렌더된다. 둘 다 `&&` 로
         독립적으로 걸려 있어서 서로를 배제하지 않는다.

       ★ 환불 정책은 전자상거래법이 요구하는 표시 항목이고, 사업자 정보도 그렇다.
         로그인해야 볼 수 있으면 표시 의무를 충족하지 못한다.
    */
    '/terms', '/privacy', '/risk', '/security', '/refund', '/company',
  ]);
