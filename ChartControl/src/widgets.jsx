...
(function () {
  ...
  window.OrderEntry = function OrderEntry({
    ...
    */
    ...
    /*
       ★★ **수량이 없으면 최소 주문금액 경고도 띄우지 않는다.**

         수량을 비웠더니(위) totalUSDT 가 0 이 되어 이 경고가 첫 화면부터 떴다.
         "수량을 입력하세요" 와 "최소 5 USDT" 가 함께 나오면 무엇을 해야 하는지
         오히려 흐려진다 — 지금 필요한 안내는 앞의 하나다.
         프로덕션에서 재보고 발견했다(요약은 — 인데 경고만 남아 있었다).

       ★ 최소금액은 **입력한 값이 너무 작을 때** 알려야 의미가 있다.
    */
    if (sz > 0 && totalUSDT < 5) errors.push({ level: 'warn', text: t('oe_err_min_notional') });
    /*
