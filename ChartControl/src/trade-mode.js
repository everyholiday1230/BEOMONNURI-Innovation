...
(function () {
  ...
  var MODES = {
    ...
    paper: {
      id: 'paper',
      /*
         ★★★ **페이퍼 모드는 아직 동작하지 않는다 — 2026-09-14 프로덕션 실측.**

           운영자 요청으로 페이퍼에서 끝까지 주문을 시도해 확인했다:

             ① 주문 검증이 "거래소 API 키를 먼저 연결하세요" 로 막았다.
                모의 거래인데 실거래 키를 요구했다(이 가드는 함께 고쳤다).
             ② 고쳐도 낼 수 없다 — **모의 시작 잔고가 없다.**
                `account_balances` 프로덕션 **0행**. 시뮬레이터에 돈을 넣는 코드가
                **시험 파일에만** 있고 제품 경로에는 없다.
             ③ 시뮬레이터 투영(`pg-sim-projection`)은 `orders`·`positions`·
                `executions` 만 쓰고 **잔고를 갱신하지 않는다.**
             ④ 증거: `simulation_orders` 0행, `orders` 0행 —
                페이퍼 주문이 **한 번도 성공한 적이 없다.**

         ★★ 그래서 `available: false` 로 둔다. 동작하지 않는 버튼을 눌러도 되는 것처럼
           두지 않는다. 버튼 자체는 남기고(디자이너 UI 계약) 준비중 표시와 이유를
           보여준다 — 이유 없이 사라지면 "있었는데 없어졌다" 가 된다.

         ★ 되살리려면 세 가지가 필요하다:
             · 가입 시(또는 요청 시) 모의 시작 잔고 지급
             · 시뮬레이터가 체결마다 잔고를 갱신
             · 주문 패널·포지션·차트가 페이퍼일 때 시뮬레이터 값을 읽도록 분기
           대회(모의 랭킹)를 하려면 반드시 필요한 작업이다.
      */
      /*
         ★★ 2026-09-14 되살렸다. 위에 적은 세 가지를 모두 만들었다:
             · 모의 시작 잔고 지급 (POST /api/sim/balance/ensure, 멱등)
             · 시뮬레이터가 체결마다 잔고를 갱신 (증거금·손익·수수료)
             · 계정 데이터가 페이퍼일 때 시뮬레이터를 읽도록 분기
           청산 전용(TP/SL) 체결도 반대쪽 포지션을 줄이도록 처리했다 — 그것이 없으면
           손절이 새 포지션을 만들어 노출이 두 배가 된다.
      */
      available: true,
      reasonKey: null,
