...
(function () {
  ...
  function timeAgo(ts) {
    // ★ ISO 문자열도 받는다(서버 시각). NaN 이면 '—' 로 말한다 — 사용자 화면과 동일한 정규화.
    const __ms = (typeof ts === 'number') ? ts : new Date(ts).getTime();
    if (!Number.isFinite(__ms)) return '—';
    const s = Math.floor((Date.now() - __ms) / 1000);
