  ...
  */
  ...
  function generateCandles(opts) {
    ...
    */
    ...
    /*
       ★★★ **실캔들이 아직 없을 때 목업을 돌려주면 안 된다.**

         운영자 신고: "종목이나 타임프레임 처음 누르면 장대 봉이 꼭 생겼다가 실시간
         차트로 돌아간다. 모든 종목이." 원인이 이 한 줄이었다.

         실측(프로덕션, BTCUSDT):
           1H 최초 호출 → 65,034~68,433   ← 목업(시드 가격 68,432.5)
           6초 뒤       → 77,880~79,661   ← 실데이터
         **약 1만 달러 차이**다. 그 간격이 한 봉으로 이어져 장대봉으로 보인다.

       ★★ 이것은 보기 흉한 정도가 아니라 **위험하다.** 실주문이 열린 제품에서
         가짜 시세를 1초라도 보여주면, 그 순간 화면을 본 고객이 존재하지 않는 급등·
         급락을 근거로 판단할 수 있다. 사라진 값을 사람은 기억한다.

       ★ 정책 함수가 이미 있다 — `QTMockPolicy.allowMockData()`.
         그 함수의 주석이 정확히 이 상황을 말한다: "null 은 아직 모른다. 그때 목업을
         허용하면 첫 렌더에 예시가 보이고 곧 실데이터로 바뀐다. 모르는 동안에는
         보여주지 않는다." 그런데 이 자리가 그 정책을 **쓰지 않고** 있었다.

       ★ 빈 배열을 돌려준다. 차트는 로딩 표시를 띄우고(window.__qtChartLoading /
         chart_loading_tf) 실데이터가 도착하면 그린다 — 이미 그 경로가 있다.

       ★ 백엔드가 없는 미리보기(디자이너 확인)에서는 그대로 목업을 쓴다. 그때는
         실데이터가 올 수 없고, 빈 차트만 보여주면 화면을 만들 수 없다.
    */
    var allowMock = true;
    try {
      var MP = window.QTMockPolicy;
      if (MP && typeof MP.allowMockData === 'function') allowMock = MP.allowMockData();
    } catch (e) { allowMock = false; }
    if (!allowMock) return [];
