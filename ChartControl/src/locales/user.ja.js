...
(function () {
  ...
  window.QTI18n.register(
    'ja',
    {
      ...
      */
      wal_revoke_confirm: 'このAPIキーを失効させますか？注文と残高の取得が直ちに停止します。取り消せません — 後で新しいキーを接続できます。',
      wal_revoke_failed: 'キーを失効できませんでした: {msg}。まだ有効です — 再試行するか、取引所側で削除してください。',

      /* ★ 예시 전략 화면의 버튼이 왜 눌리지 않는지 말한다. 회색 버튼만으로는 이유를 알 수 없다. */
      strat_needs_key: '利用するには取引所APIキーの接続が必要です — これは例のストラテジーです。',
    },
