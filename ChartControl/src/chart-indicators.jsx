  ...
  function applySavedIndicators(chart, saved) {
    ...
    /*
       ★★★ **복원 뒤에 게시해야 범례가 갱신된다.**
         ...
         복원 자체는 성공하는데(실측: 차트 지표 ["MA","VOL","MACD"]) 화면 범례가
         갱신되지 않았다. 범례는 `QTChartState.getIndicatorDetail()` 을 구독하고,
         그 값을 게시하는 것은 지표 패널의 syncFromChart 인데 패널이 **닫혀 있으면
         실행되지 않는다.**

       ★★ 게시는 **차트가 해야 한다** — `ChartKlineUtil.publishState()` 가 이미 그
         일을 한다(지표값·설정값까지 담는다). 내가 여기서 직접 게시했더니 차트의
         게시와 두 벌이 되어 서로 덮어썼다. 소유자가 하나여야 한다.

       ★ 게시는 다음 tick 에 부른다 — createIndicator 직후에는 차트가 아직 계산을
         마치지 않아 값이 비어 있다.
    */
    if (added) {
      setTimeout(function () {
        try {
          if (window.ChartKlineUtil && window.ChartKlineUtil.publishState) {
            window.ChartKlineUtil.publishState();
          }
        } catch (e) {
