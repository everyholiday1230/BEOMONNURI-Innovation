...
(function () {
  ...
  var WIRED_SELECTORS = [
    ...
    '.modal button',
    /*
       ★★ '.admin-shell button' 을 여기서 뺐다.

         관리자 화면 전체를 "배선된 영역" 으로 잡고 있었다. 그런데 아래 isWired()
         는 하드 early-return 이라, 이 선택자에 걸리면 **관찰조차 하지 않는다.**
         그 결과 배선되지 않은 관리자 버튼(예: 브로드캐스트 '발송', 승인/거부,
         프롬프트 '배포')이 조용히 죽었다 — 운영자는 발송된 줄 알고 화면을 떠난다.
         공지를 안 보낸 채 보냈다고 믿는 것이 이 화면에서 가장 위험한 실패다.

         관리자 화면의 **실제로** 배선된 버튼들은 클릭하면 행·모달·토스트가
         바뀌므로 아래의 결과 관찰(스냅샷 비교)이 정상적으로 통과시킨다.
    */
  ];
