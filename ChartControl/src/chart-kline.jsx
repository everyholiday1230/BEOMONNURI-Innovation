...
(function () {
  ...
  */
  const TF_MS = {
    ...
    '1H': 3600e3, '2H': 7200e3, '4H': 14400e3,
    /*
       ★★ 새 주기를 여기에도 넣어야 한다. 빠지면 `TF_MS[tf]` 가 `undefined` 가 되어
         "이 캔들이 그 주기의 것인가" 판정이 **조용히 어긋난다** — 차트가 엉뚱한 봉을
         그대로 그린다.
       ★ 화면은 대문자(`6H`), 서버·API 는 소문자(`6h`)를 쓴다. 양쪽을 다 넣는다 —
         어느 쪽이 들어와도 판정이 되어야 한다.
    */
    '6H': 21600e3, '8H': 28800e3, '12H': 43200e3,
    '6h': 21600e3, '8h': 28800e3, '12h': 43200e3,
    '1D': 86400e3, '1W': 604800e3,
    /* ★ 한 달은 길이가 고정이 아니다(28~31일). 판정용으로 30일을 쓴다. */
    '1M': 2592000e3,
  };
  ...
  function candlesMatchTimeframe(bars, timeframe) {
    ...
    /*
       ★★★ **길이가 고정이 아닌 주기는 정확히 같을 수 없다.**

         예전에는 `mode === want` 였다. 월봉을 넣자마자 차트가 **영원히 로딩 중**이
         됐다(운영자 보고, 실측 0봉). 한 달은 28·30·31일이 섞여 있어서 우리가 정한
         30일과 **절대 같지 않다.** 그래서 매번 "이 캔들은 그 주기가 아니다" 로 판정해
         데이터를 버렸다.

       ★★ 그렇다고 **모든 주기에 여유를 주면 안 된다.** 0.5~2배로 열어 보니
         `15m↔30m`·`1H↔2H`·`12H↔1D` 같은 **2배 조합이 전부 통과**했다 — 그러면 이
         검사가 존재하는 이유가 없어진다(엉뚱한 주기의 봉을 그대로 그린다).

       ★ 그래서 **길이가 변하는 주기만** 여유를 준다. 달(28~31일)과 주(서머타임 경계에서
         한 시간 어긋날 수 있다)뿐이다. 나머지는 고정 길이이므로 정확히 같아야 한다.
       ★ 달의 여유는 ±20% 다. 28/30 = 0.93, 31/30 = 1.03 이므로 충분하고, 가장 가까운
         다른 주기인 1주(0.23배)·1일(0.03배)과는 여전히 명확히 구별된다.
    */
    const VARIABLE = { '1M': 0.2, '1W': 0.05 };
    const slack = VARIABLE[timeframe];
    if (slack !== undefined) {
      const ratio = mode / want;
      return ratio >= 1 - slack && ratio <= 1 + slack;
    }
    return mode === want;
  }
